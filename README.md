...
El proyecto ya incluye los plugins y dependencias necesarias (`com.google.gms.google-services`, `firebase-bom`, Firestore y Analytics). Solo falta enlazarlo con tu proyecto de Firebase `serene-ae012`.
...
6. Para habilitar Firestore:
   - En la consola de Firebase, abre **Firestore Database** y crea la base de datos en modo de prueba o producción según prefieras.
   - Crea una colección llamada `diary_entries` (puedes hacerlo manualmente o desde la app una vez implementes la persistencia).
7. (Opcional) Activa **Firebase Analytics** en la consola y revisa que los eventos se registren correctamente cuando ejecutes la app.

Una vez que `google-services.json` esté presente, la clase `SereneApplication` inicializará automáticamente Firebase al arrancar la app. El repositorio `DiaryRepository` ya incluye comentarios `TODO` donde deberás conectar las operaciones de Firestore (`add`, `update`, `delete`, `listen`).
